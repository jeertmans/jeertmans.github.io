...
"""
        ...
        title = VGroup(
            Tex(
                r"\textbf{Differentiable Ray Tracing\\ for \\ Telecommunications}",
                font_size=60,
            ),
            Tex("Jérome Eertmans - July 17th, Siegen"),
        ).arrange(DOWN, buff=1)
        ...
        about = about_title, profile, interests, claude, laurent = VGroup(
            ...
            Tex(
                r"""Profile:
                \begin{itemize}
                \item PhD student at UCLouvain since 09/2021;
                \item Electromechaninal Engineer in mechatronics;
                \item introduced to Ray Tracing (RT)\\during a student job.
                \end{itemize}
                """,
                font_size=30,
            ...
            Tex(
                ...
                \begin{itemize}
                \item Programming;
                \item Writing performant solutions;
                \item and open sourcing content\\(jeertmans on \faGithub{} or eertmans.be).
                \end{itemize}
                """,
                font_size=30,
                tex_environment=None,
            ...
            .to_corner(UR)
            .shift(1.5 * DOWN),
            Tex("Claude Oestges", font_size=40).to_corner(DL).shift(2 * UP),
            Tex("Laurent Jacques", font_size=40),
        )
        ...
        self.play(FadeIn(about_title))

        self.next_slide()

        self.play(FadeIn(about[1:]), FadeIn(claude_image), FadeIn(laurent_image))
        ...
        self.play(
            FadeOut(about),
            FadeOut(claude_image),
            FadeOut(laurent_image),
            self.update_slide_number(),
        )

        # Slide: introduction to RT
        ...
        _, sec1, sec2, sec3, sec4, sec5 = outline = VGroup(
            Tex(r"\textbf{Outline:}"),
            Tex("1. RT for Telecom"),
            Tex("2. Image Method"),
            Tex("3. Min-Path-Tracing"),
            Tex("4. About differentiability"),
            Tex("5. Current work"),
        ).arrange(DOWN)
        ...
        # Slide: about RT in telecom

        text_in_telecom = Tex(
            r"""Core points:
        \begin{itemize}
            \item used by telecom operators;
            \item for coverage map;
            \item for antennas placement;
            \item using mix of Ray-Launching\\ and Image RT.
        \end{itemize}""",
            font_size=40,
            tex_environment=None,
        ).next_to(W2, RIGHT)
        ...
        self.next_slide()
        self.play(FadeIn(text_in_telecom))
        ...
        self.next_slide()
        self.play(
            self.update_slide_number(), Transform(sec1, sec2), FadeOut(text_in_telecom)
        )
