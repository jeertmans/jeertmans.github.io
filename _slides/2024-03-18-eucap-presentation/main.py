...
class Main(Slide):
    ...
    def construct(self):
        ...
        # Discontinuity images: show how reducing max_depth / disabling diffraction
        # creates holes / discontinuities. Images are generated by the
        # discontinuity-illustration script in the private-defense slides.
        self.next_slide(notes="Discontinuity: limiting paths and diffraction")
        discont_folder = "../2026-05-05-private-defense/images/discontinuity"
        discont_images = [
            f"{discont_folder}/depth3_diffraction.png",
            f"{discont_folder}/depth3_nodiffraction.png",
            f"{discont_folder}/depth2_nodiffraction.png",
            f"{discont_folder}/depth1_nodiffraction.png",
        ]

        # Start with the first image, then fade each subsequent one in.
        img = ImageMobject(discont_images[0])
        self.play(
            self.next_slide_number_animation(),
            self.wipe(self.mobjects_without_canvas, img, return_animation=True),
        )

        for path in discont_images[1:]:
            next_img = ImageMobject(path)
            self.play(FadeOut(img), FadeIn(next_img))
            img = next_img

        # Show the zero-gradient example (simple setup) after the sequence
        self.next_slide(notes="Zero-gradient example (simple setup)")
        zg = ImageMobject("zero_gradient.png")
        self.play(
            self.next_slide_number_animation(),
            self.wipe(self.mobjects_without_canvas, zg, return_animation=True),
        )
        ...
        # Math intro to smoothing
        self.next_slide(notes="Definition of the smoothing function")
        smoothing_def = MathTex(
            r"s(x;\alpha)=\frac{1}{1+e^{-\alpha x}}",
            r"\quad\text{with }\lim_{\alpha\to\infty}s(x;\alpha)=\theta(x)",
        )
        self.play(
            self.next_slide_number_animation(),
            self.wipe(
                self.mobjects_without_canvas, smoothing_def, return_animation=True
            ),
        )
